...
def test_browse_query_carries_min_year_floor():
    # Sending min_year is required for full auction-house coverage (omitting it
    # returns a Kantou-skewed subset). The exact floor is under test (1970 to try
    # to also capture pre-1990 bikes); assert whatever BROWSE_MIN_YEAR is set to.
    s, urls = _capture_urls({1: ["1001"], 2: []})
    s.browse_make_urls("Honda", 2000)
    assert all(f"min_year={sc.BROWSE_MIN_YEAR}" in u for u in urls), urls
