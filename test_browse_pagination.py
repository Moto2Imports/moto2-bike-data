...
def test_counter_dedupes_multi_anchor_and_walks_pages():
    ...
    assert s.count_make_listings("Honda") == 5
...
def test_counter_terminates_on_last_page_reserve():
    ...
    assert s.count_make_listings("Yamaha") == 3
...
def test_counter_is_independent_of_seen_state():
    ...
    assert s.count_make_listings("Suzuki") == 2
...
def test_two_makes_share_dedup_across_the_run():
    ...
    first = s.browse_make_urls("Honda")
    second = s.browse_make_urls("Honda")             # same lots, already claimed
